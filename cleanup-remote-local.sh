...
# Remote Local Audio Cleanup Script
# Runs cleanup on production server 31.184.134.153 from local machine
# Usage: ./cleanup-remote-local.sh [--dry-run] [--days N]
...
check_cleanup_script() {
    ...
    if ! remote_exec "test -f $SERVER_PROJECT_DIR/cleanup-local-audio.sh"; then
        log_error "Cleanup script not found on server at $SERVER_PROJECT_DIR/cleanup-local-audio.sh"
...
make_script_executable() {
    ...
    remote_exec "chmod +x $SERVER_PROJECT_DIR/cleanup-local-audio.sh"
...
# Function to show conversations directory size
show_conversations_size() {
    log "Checking conversations directory size on server..."
...
# Main execution
main() {
    log "Starting remote local audio cleanup script"
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        log_success "Remote local audio cleanup completed successfully"
    fi
